from lex_analyze import lex, tableToPrint
from lex_analyze import tableOfSymb, tableOfIdents, tableOfConst, tableOfLabel, sourceCode, FSuccess
...
def parseProgram():
    ...
    try:
        ...
        parseToken('startprogram', 'keyword', '')  # Трансляція не потрібна  # лексема не має операційної семантики
        parseToken('\n', 'eol', '')
        # перевірити синтаксичну коректність списку інструкцій StatementList
        parseStatementList()  # Трансляція (тут нічого не  робити)
        # ця функція сама згенерує  # та додасть ПОЛІЗ інструкцій  (виразів)
        # перевірити наявність ключового слова 'end'
        parseToken('endprogram', 'keyword', '')  # Трансляція не потрібна  # лексема не має операційної  семантики
...
def parseToken(lexeme, token, indent):
    ...
    if (lex, tok) == (lexeme, token):
        # вивести у консоль номер рядка програми та лексему і токен  # print(indent+'parseToken: В рядку {0} токен {1}'.format(numLine,(lexeme,token)))
        return True
    else:
...
def getSymb():
    if numRow > len_tableOfSymb:
        failParse('getSymb(): неочікуваний кінець програми',
                  numRow)  # таблиця розбору реалізована у формі словника (dictionary)  # tableOfSymb[numRow]={numRow: (numLine, lexeme, token, indexOfVarOrConst)
...
def parseStatement():
    ...
    elif (lex, tok) == ('endprogram', 'keyword'):
        return False
    else:
...
def parseEndLine():
    ...
    if lex == '\n':
        ...
        failParse('неправильний кінець рядка', (numLine, lex, tok, '\n'))
        return False
...
def parseAssign():
    ...
    numRow += 1
    # print('\t'*5+'в рядку {0} - {1}'.format(numLine,(lex, tok)))  # якщо була прочитана лексема - ':='
    if parseToken('=', 'assign_op', '\t\t\t\t\t'):
        # розібрати арифметичний вираз
        parseExpression()  # Трансляція (тут нічого не робити)   # ця функція сама згенерує
        # та додасть ПОЛІЗ виразу
        postfixCode.append(('=', 'assign_op'))  # Трансляція
        # Бінарний оператор ':='
        # додається після своїх операндів  if toView: configToPrint('=',numRow)
        return True
    else:
        return False
...
def parseExpression():
    ...
    while F:
        ...
        if tok in ('add_op'):
            ...
            parseTerm()  # Трансляція (тут нічого не робити)   # ця функція сама згенерує
            # та додасть ПОЛІЗ доданка
...
def parseTerm():
    global numRow, postfixCode
    # print('\t'*6+'parseTerm():')
    parseFactor()  # Трансляція (тут нічого не робити)   # ця функція сама згенерує
    ...
    while F:
        ...
        if tok in ('mult_op', 'exponent'):
            numRow += 1
            # print('\t'*6+'в рядку {0} - {1}'.format(numLine,(lex, tok)))
            parseFactor()  # Трансляція (тут нічого не робити)   # ця функція сама згенерує та додасть ПОЛІЗ  множника
            # Трансляція
            postfixCode.append((lex, tok))
            # lex - бінарний оператор '*' чи '/'  # додається після своїх операндів
            if toView: configToPrint(lex, numRow)
...
def parseFactor():
    ...
    if tok in ('int', 'float', 'ident'):
        postfixCode.append((lex, tok))  # Трансляція
        # ПОЛІЗ константи або ідентифікатора  # відповідна константа або ідентифікатор
        if toView: configToPrint(lex, numRow)
        numRow += 1
...
def parseWrite():
    ...
    parseToken('(', 'brackets_op', '\t' * 7)
    parseExpression()
    postfixCode.append(('write', 'Out'))
    k = True
    while (k):
        k = readWriteMany('write', "Out")
    parseToken(')', 'brackets_op', '\t' * 7)
    return True
...
def parseRead():
    ...
    parseToken('(', 'brackets_op', '\t' * 7)
    parseExpression()
    postfixCode.append(('read', 'Inp'))
    k = True
    while (k):
        k = readWriteMany('read', "Inp")
    parseToken(')', 'brackets_op', '\t' * 7)
    return True
